...
export class ParseColor {
    ...
    static getStylePropertyFromElement(selector, property) {
        const el = document.querySelector(selector);
        const style = window.getComputedStyle(el);
        return style.getPropertyValue(property);
    }

    static getColorArrayFromElement(selector, property) {
        return ParseColor.parseColor(ParseColor.getStylePropertyFromElement(selector,property));
    }

    static getColorValueFromElement(selector, property) {
        const rgb = ParseColor.getColorArrayFromElement(selector, property);
        return (rgb[0] << 16) + (rgb[1] << 8) + rgb[2];
    }

    static getColorFloatArrayFromElement(selector, property) {
