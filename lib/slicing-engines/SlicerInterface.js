...
class SlicerInterface {
     ...
     */
    async loadProfile(type, filename) {
        return fetchText("config/syndaver/" + type + "_profiles/" + filename)
               .then(data => {console.log("Loaded profile", filename); this.config.loadProfileStr(data);});
    }
}
