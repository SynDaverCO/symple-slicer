...
class CuraConfiguration {
    ...
    loadDefaults(force) {
        ...
        for(const [key, node] of Object.entries(this.nodes)) {
            var affected = 0;
            if(node.hasOwnProperty('default_value') && (this.values[key] != node.default_value)) {
                this.values[key] = node.default_value;
            }
            if(force) {
                affected = CuraConfiguration.VALUE_AFFECTED |
                          CuraConfiguration.FLAGS_AFFECTED;
            }
            affected = this.recomputeValue(key, affected);
            affected = this.recomputeFlags(key, affected);
            this.setFlag(key, CuraConfiguration.CHANGED_FLAG, false);
            affectedSettings[key] = affected;
        }
    ...
    postProcessAffectedSettings(affectedSettings) {
        for(const key of Object.keys(affectedSettings).sort()) {
            if (affectedSettings[key] & CuraConfiguration.VALUE_AFFECTED) {
                var val = this.values[key];
                if(this.verbose)
                    console.log("--> Changed", key, "to", val);
                this.onValueChanged(key, val);
            }
            if (affectedSettings[key] & CuraConfiguration.FLAGS_AFFECTED) {
        ...
        for(const key of excluding) {
            ...
            if(this.values[key] != val) {
                ...
                if(this.verbose)
                    console.log("Changed", key, "to", val);
                this.onValueChanged(key, val);
                this.setFlag(key, CuraConfiguration.CHANGED_FLAG, true);
            }
        }
        this.postProcessAffectedSettings(affectedSettings);
    }
    ...
    recomputeValue(key, affected) {
        ...
        var val  = node.hasOwnProperty('resolve') ? this.evaluatePythonExpression(key, node.resolve) :
                   ...
                   node.default_value;
        if (this.values[key]    != val) {affected |= CuraConfiguration.VALUE_AFFECTED; this.values[key] = val;}
        return affected;
    }
    ...
    recomputeFlags(key, affected) {
        ...
        if (this.isEnabled(key) != en)  {affected |= CuraConfiguration.FLAGS_AFFECTED; this.setFlag(key, CuraConfiguration.ENABLED_FLAG, en);}
        return affected;
    }
    ...
    saveProfileStr(attr) {
        ...
        for(const key of Object.keys(this.nodes).sort()) {
            ...
            var modified = this.getFlag(key, CuraConfiguration.CHANGED_FLAG);
...
CuraConfiguration.CHANGED_FLAG  = 2; // Set if a value was changed from the default

CuraConfiguration.VALUE_AFFECTED = 1;
CuraConfiguration.FLAGS_AFFECTED = 2;