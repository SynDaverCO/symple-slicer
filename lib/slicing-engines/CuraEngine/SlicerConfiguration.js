...
class SlicerConfiguration {
    ...
    propagateChanges(key, excluding, changes) {
        ...
        this.getDependents(key).forEach(
            dependent => {
                var whatChanged = 0;
                if(this.nodeDependsOn(dependent, key, "value") && excluding.indexOf(dependent) == -1) {
    ...
    saveProfileStr(attr) {
        ...
        for(const key of Object.keys(this.nodes).sort()) {
            ...
            var changed = this.getFlag(key, SlicerConfiguration.CHANGED_FLAG);

            if(!changed && attr && !attr.unchanged)
                continue;

            // Print unchanged values as comments.
            key_str = ((changed ? "" : "#") + key).padEnd(key_width + 1);
            if(!changed) {
