...
class ToolboxUI {
	...
	page(id, menuText) {
		...
		if(menuText) {
			ToolboxUI.addTag(this.menu, "option", {innerHTML: menuText, value: id});
		}
		// Create the page itself and make it the target of future DOM insertions.
		this.target_dom = ToolboxUI.addTag(this.ui, "div", {id: id, className: "toolbox-panel"});
	}
	...
	parameter(id, description, default_value) {
		...
		var container = ToolboxUI.addTag(this.target_dom, "div", {className: "parameter"});
		ToolboxUI.addTag(container, "label",  {innerHTML: description});
		var el = ToolboxUI.addTag(container, "input", {id: id});
	...
	choice(id, description) {
		var container = ToolboxUI.addTag(this.target_dom, "div", {className: "parameter"});
		ToolboxUI.addTag(container, "label",  {innerHTML: description || "&nbsp;"});
		var el = ToolboxUI.addTag(container, "select", {id: id});
		return {
			option: function(id, text) {
				var o = ToolboxUI.addTag(el, "option", {innerHTML: text});
				o.value = id;
				return this;
			}
		}
	...
	heading(text) {
		ToolboxUI.addTag(this.target_dom, "h1", {innerHTML: text});
	}
	...
	button(func, label, className) {
		ToolboxUI.addTag(this.target_dom, "button", {innerHTML: label, className: className, onclick: func});
	}
	
	buttonHelp(text) {
		ToolboxUI.addTag(this.target_dom, "div", {innerHTML:text, className: "button-label"});
	}
	
	textarea(id) {
		var container = ToolboxUI.addTag(this.target_dom, "div", {className: "parameter"});
		return ToolboxUI.addTag(container, "textarea", {id: id});
	}
	
	div(id) {
		if(typeof id === 'string' || id instanceof String) {
			ToolboxUI.addTag(this.target_dom, "div", {id: id});
		} else {
	...
	file(id, binary) {
		var container = ToolboxUI.addTag(this.target_dom, "div", {className: "parameter drop-box empty"});
		var el = ToolboxUI.addTag(container, "input", {id: id});
		el.type = "file";
				
		// Drop area
		var da   = ToolboxUI.addTag(container, "div", {className: "drop-area"});
		ToolboxUI.addTag(da, "div", {innerHTML: "Drop file here"});
		
		// Selected file & reset button
		var sf = ToolboxUI.addTag(container, "div", {className: "selected-file"});
		var selectedFile = ToolboxUI.addTag(sf, "span");
		ToolboxUI.addTag(sf, "div", {
            innerHTML: "&#x2716;",
            className: "reset",
