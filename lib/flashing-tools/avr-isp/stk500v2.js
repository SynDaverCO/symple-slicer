        ...
        for (let i = 0; i < Math.floor(load_count); i++) {
            ...
            if (this.progress_callback) {
                if (this._has_checksum) {
                    this.progress_callback(i + 1, load_count)
                } else {
                    this.progress_callback(i + 1, load_count * 2)
                }
            }
        }
        console.log("...DONE");
    ...
    async verifyFlash(flash_data) {
        ...
        if (this._has_checksum) {
            ...
            for (let i = 0; i < load_count; i++) {
                ...
                if (this.progress_callback) {
                    this.progress_callback(load_count + i + 1, load_count * 2);
                }
