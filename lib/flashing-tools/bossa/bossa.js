...
export class BOSSA {
    ...
    async flash_by_port(port, data) {
        await this.connect(port);
        await this.flash_firmware(data);
    }

    async flash_by_id(vendorId, productId, data) {
        const ports = await SequentialSerial.getPorts();
        for(let i = 0; i < ports.length; i++) {
            if(ports[i].vendorId == vendorId && ports[i].productId == productId) {
                await this.flash_by_port(ports[i].path, data);
                return;
            }
        }
