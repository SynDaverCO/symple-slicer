...
class GCodeParser {
    ...
    parse(obj) {
        ...
        var setPosRe   = /^G92\s+([XYZFE][0-9\.\-]+\s*)+/;
        var modeRe     = /^G9([01])\b/;
        var emodeRe    = /^M83\b/;
        ...
        var processSetPos  = obj.hasOwnProperty("setPos");
        ...
        this.data.match(lineRe).forEach(
            line => {
                if (m = movementRe.exec(line)) {
                    ...
                    parseAxis(
                        m,
                        v => {processSetPos && obj.setPos("X", v); x = v;},
                        v => {processSetPos && obj.setPos("Y", v); y = v;},
                        v => {processSetPos && obj.setPos("Z", v); z = v;},
                        v => {processSetPos && obj.setPos("E", v); e = v;}
                    );
                }
            }
        );
