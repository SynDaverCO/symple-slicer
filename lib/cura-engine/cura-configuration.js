...
class CuraConfiguration {
    ...
    setMultiple(settings) {
        var excluding = Object.keys(settings);
        // Update all the changed settings
        var changedSettings = {};
        for(const [key, value] of Object.entries(settings)) {
            if(this.isEnabled(key) && this.values[key] != value) {
                this.values[key]     = value;
                changedSettings[key] = value;
            }
        }
        // Propagate changes to decendants
        var affectedSettings = {};
        for(const key of Object.keys(changedSettings).sort()) {
            console.log("Changed", key, "to", changedSettings[key]);
            this.propagateChanges(key, excluding, affectedSettings);
        }
        for(const key of Object.keys(affectedSettings).sort())
            console.log("--> Changed", key, "to", affectedSettings[key]);
    }
     ...
     * in the exclusion list. Returns an object with all changes that were made.
     */
    propagateChanges(key, excluding, changedSettings) {
        changedSettings = changedSettings || {};
        this.getDependents(key).forEach(
            dependent => {
                if(excluding.indexOf(dependent) == -1 && this.isEnabled(dependent) && this.recomputeValue(dependent)) {
                    changedSettings[dependent] = this.values[dependent];
                    this.propagateChanges(dependent, excluding, changedSettings);
                }
            }
        );
    ...
    /**
     * Recomputes the value of a setting, returns true if the settings value changed.
     */
    recomputeValue(key) {
        ...
        var val = node.hasOwnProperty('resolve') ? this.evaluatePythonExpression(key, node.resolve) :
                  ...
                  node.default_value;
        var changed = this.values[key] != val;
        this.values[key] = val;
        return changed;
    }
    ...
    isEnabled(key) {
        var node = this.nodes[key];
        if(!node) {
            console.log("Warning: Unrecognized setting", key);
            return false;
        }
        return node.hasOwnProperty('enabled') ? this.evaluatePythonExpression(key, node.enabled) : true;
    }
     ...
     */
    loadProfileOverrides(type, filename) {
