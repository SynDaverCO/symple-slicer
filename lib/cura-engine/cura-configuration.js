...
class CuraConfiguration {
    ...
    onAttributesChanged(key, attributes) {
    }
    ...
    loadCuraJSONDefinitions(fileList) {
        ...
        for(const filename of fileList) {
            fetchJSON(filename, this.onDataReady.bind(this, filename), this.onDataError.bind(this, filename));
        }
    }
    ...
    loadDefaults(force) {
        ...
        for(const [key, node] of Object.entries(this.nodes)) {
            if(node.hasOwnProperty('default_value') && (force || this.values[key] != node.default_value)) {
                this.values[key] = node.default_value;
                affectedSettings[key] = CuraConfiguration.VALUE_CHANGED;
            }
            this.recomputeValue(key);
            this.recomputeFlags(key);
            this.setFlag(key, CuraConfiguration.MODIFIED_FLAG, false);
        }
        ...
        for(const key of excluding) {
            ...
            if(this.values[key] != val) {
                ...
                if(this.verbose)
                    console.log("Changed", key, "to", val);
                this.onValueChanged(key, val);
                this.setFlag(key, CuraConfiguration.MODIFIED_FLAG, true);
            }
        }
        this.postProcessAffectedSettings(affectedSettings);
    }
    ...
    /**
     * Recomputes the value of a setting, returns changed with new bits set representing any attributes that changed.
     */
    recomputeValue(key, changed) {
        ...
        var val  = node.hasOwnProperty('resolve') ? this.evaluatePythonExpression(key, node.resolve) :
                   ...
                   node.default_value;
        if (this.values[key]    != val) {changed |= CuraConfiguration.VALUE_CHANGED; this.values[key] = val;}
        return changed;
    }
    
    /**
     * Recomputes the flags of a setting, returns changed with new bits set representing any attributes that changed.
     */
    recomputeFlags(key, changed) {
        var node = this.nodes[key];
        var en   = node.hasOwnProperty('enabled') ? this.evaluatePythonExpression(key, node.enabled) : true;
        if (this.isEnabled(key) != en)  {changed |= CuraConfiguration.ENABLE_CHANGED; this.setFlag(key, CuraConfiguration.ENABLED_FLAG, en);}
        return changed;
    }
     ...
     */
    isEnabled(key) {
        return this.getFlag(key, CuraConfiguration.ENABLED_FLAG);
    }
    ...
    /**
     * Generates an TOML file with all the current settings.
     *
     * Attributes:
     *   defaults     - Whether to include unmodified defaults in the config file
     *   descriptions - Whether to annotate the file with descriptions
     *   choices      - Whether to annotate the file with units and choices
     */
    saveProfileStr(attr) {
        // Compute the length of the longest setting.
        var key_width = 0;
        ...
        for(const key of Object.keys(this.nodes).sort()) {
            ...
            var modified = this.getFlag(key, CuraConfiguration.MODIFIED_FLAG);

            if(!modified && attr && !attr.defaults)
                continue;

            // Make unconfigured values comments.
            key_str = ((modified ? "" : "#") + key).padEnd(key_width + 1);
            ...
            // Append the configuration line to the string
            str += key_str + " = " + (comments ? val_str.padEnd(21, " ") + " # " + comments : val_str) + "\n";
        }
        return str;
    }
}
...
CuraConfiguration.ENABLED_FLAG   = 1; // Set if a value is enabled in the UI
CuraConfiguration.MODIFIED_FLAG  = 2; // Set if a value was modified from the default

CuraConfiguration.VALUE_CHANGED  = 1;
CuraConfiguration.ENABLE_CHANGED = 2;