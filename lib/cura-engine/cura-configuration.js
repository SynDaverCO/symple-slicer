...
class CuraConfiguration {
    ...
    onDataReady(json) {
        ...
        // Second pass: Check all expressions and store pre-requisites.
        ...
        cfg.parseValue = (key, expr) => {
            var computedValue = preprocessSetting(key, expr);
            var defaultValue  = this.values[key];
            if(computedValue != defaultValue) {
                //console.log("Updating default value for", key, "of", defaultValue , "with computed value of", computedValue);
                this.values[key] = computedValue;
            }
        };
    ...
    set(key, value) {
        if(this.values[key] != value) {
            ...
            var changes = this.propagateChanges(key);
    ...
    /**
     * Recomputes the value of all settings which depend on the key. Returns
     * an object with a list of all settings changes that were made.
     */
    propagateChanges(key, changedSettings) {
        changedSettings = changedSettings || {};
        this.getDependents(key).forEach(
            dependent => {
                if(this.isEnabled(dependent) && this.recomputeValue(dependent)) {
                    changedSettings[key] = this.values[key];
                    this.propagateChanges(dependent, changedSettings);
                }
            }
        );
