...
self.importScripts('cura-args.js');
...
function loadGeometry(geometry) {
    try {
        self.postMessage({'cmd': 'stderr', 'str': "Writing binary STL"});
...
function receiveMessage(e) {
    ...
    switch (cmd) {
        ...
        case 'sliceFromUrl':  loadFromUrl(data.url);                   slice(); get_file(); break;
        case 'sliceFromBlob': loadFromBlob(data.blob);                 slice(); get_file(); break;
        case 'sliceGeometry': loadGeometry(jsonToGeometry(data.data)); slice(); get_file(); break;
