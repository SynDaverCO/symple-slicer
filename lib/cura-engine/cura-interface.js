...
function SlicerInterface() {
	...
	this.sliceFromUrl = function(url) {
		worker.postMessage({'cmd': 'sliceFromUrl', 'url': url});
	}
	
	this.sliceFromBlob = function(blob) {
		worker.postMessage({'cmd': 'sliceFromBlob', 'blob': blob});
	}
	
	this.sliceGeometry = function(geometry) {
		var json = geometryToJSON(geometry);
		worker.postMessage({'cmd': 'sliceGeometry', 'data': json.data}, json.tranferables);
	}
