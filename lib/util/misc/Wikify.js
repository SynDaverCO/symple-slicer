...
function wikify (str) {
    var jsonStorage = [];
    str = re_json(formatting(tables(links(figs(headers_sl(def_lists(preformatted(lists(lists(lists(lists(lists(paragraphs(headers_ml(un_json(crlf(str),jsonStorage))))))))))))))),jsonStorage);
    return str;
}