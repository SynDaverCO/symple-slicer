...
self.importScripts('../../three/3MFLoader.js');
self.importScripts('../../three/BufferGeometryUtils.js');
self.importScripts('../../jszip/jszip.min.js');
...
function load(filename, data) {
    ...
    switch(extension) {
      case 'obj': geometry = loadFromObj(data); break;
      case '3mf': geometry = loadFrom3MF(data); break;
      default:    geometry = loadFromSTL(data); break;
    }
    return geometry;
}
...
function loadFromSTL(data) {
    self.postMessage({cmd: 'progress', value: 0/3});
    var geometry = GEOMETRY_READERS.readStl(data, GEOMETRY_READERS.THREEGeometryCreator);
    self.postMessage({cmd: 'progress', value: 1/3});
    geometry.mergeVertices();
    self.postMessage({cmd: 'progress', value: 2/3});
    var bufferGeometry = geometryToIndexedBufferGeometry(geometry);
    geometry.dispose();
    self.postMessage({cmd: 'progress', value: 3/3});
...
function receiveMessage(e) {
    ...
    var data = e.data;
    switch (cmd) {
        case 'load': send(load(data.filename, data.data)); break;
