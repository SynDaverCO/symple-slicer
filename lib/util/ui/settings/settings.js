...
class SettingsUI {
    ...
    static _input(container, type, attr) {
        var input_attr = {type: type, autocomplete: "off"};
        for(const p of ["id", "onclick", "oninput", "onchange", "min", "max", "step", "checked", "value", "className"]) {
    ...
    page(menuText, attr) {
        ...
        this.target_dom = SettingsUI.addTag(this.ui, "div", attr);
        this.target_dom.classList.add("settings-panel");
        this.target_dom.classList.add(attr.id);
        this.page_id = attr.id;
    }

    footer() {
        this.category();
        this.page(null, {id: this.page_id, className: "footer settings-panel"});
        this.separator();
    }

    toggle(description, attr) {
        var container = SettingsUI.addTag(this.target_dom, "div");
        var container = SettingsUI.addTag(container, "span", {className: "parameter"});
    ...
    number(description, attr) {
        var container = SettingsUI.addTag(this.target_dom, "div", {className: "tabbable-param"});
        var container = SettingsUI.addTag(container, "span", {className: "parameter"});
        SettingsUI._label(container, description, attr);
    ...
    _lastButtonGroup() {
        ...
        if (lastChild && lastChild.classList.contains("button-group")) {
            console.log("Appending to button group");
            return lastChild;
        }
        console.log("Adding button group");
